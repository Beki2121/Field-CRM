...
export default function AddBusinessForm({ onSave, onCancel }) {
  const [form, setForm] = useState(() => {
    try {
      return (
        JSON.parse(
          localStorage.getItem("field_crm_draft_business") || "null",
        ) || {
          businessName: "",
          sector: "",
          contactPerson: "",
  ...
  useEffect(
    () =>
      localStorage.setItem("field_crm_draft_business", JSON.stringify(form)),
    [form],
  );
  ...
  const submit = async () => {
    ...
    try {
      await onSave(form);
      localStorage.removeItem("field_crm_draft_business");
    } finally {
      setSaving(false);
    }
  };

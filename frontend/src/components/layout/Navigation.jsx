      ...
      {items.map((item) => {
        ...
        if (item.primary) {
          return (
            <button
              key={item.id}
              onClick={() => goto("addVisit")}
              className="flex flex-col items-center justify-center -mt-5"
            >
              <span
                className="w-12 h-12 rounded-full flex items-center justify-center shadow-lg active:scale-95 transition"
        ...
        return (
          ...
          >
            ...
            {item.id === "followups" && pendingCount > 0 && (
              <span
                className="absolute top-0 right-2 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center"
                style={{ background: C.clay, color: "#fff" }}
              >
                {pendingCount}
              </span>

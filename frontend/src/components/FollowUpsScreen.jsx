...
export function RescheduleModal({ target, onClose, onSave }) {
  ...
  const [method, setMethod] = useState(
    target.visit.nextFollowUpMethod || "Phone",
  );
  ...
  const save = async () => {
    setSaving(true);
    try {
      await onSave(target.visit.id, { date, action, method });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ModalShell
      title={`Reschedule — ${target.business.businessName}`}
      onClose={onClose}
    >
      <Field label="Next action">
        <ChipGroup
          options={NEXT_ACTIONS.filter((a) => a !== "No Action")}
          value={action}
          onChange={setAction}
          columns={3}
        />
      </Field>
      <Field label="New follow-up date" required>
        <TextInput
          type="date"
          value={date}
    ...
    <div>
      ...
      {active.data.length === 0 ? (
        <Card>
          <EmptyState
            icon={CalendarClock}
            title={`No ${active.label.toLowerCase()} follow-ups`}
          />
        </Card>
      ) : (
          ...
          {active.data.map((f) => (
            <Card key={f.visit.id} className="p-3.5">
              <div
                ...
                onClick={() => goto("profile", f.business.id)}
              >
              ...
              >
                ...
                >
                  {f.visit.nextAction} ·{" "}
                  {fmtDateShort(f.visit.nextFollowUpDate)} ·{" "}
                  {f.visit.nextFollowUpMethod}
                </span>
                {tab !== "completed" && (
